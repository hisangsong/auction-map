...
- **기능**: 시/도 → 시군구 2단계 선택, 용도별 필터, 유찰횟수별 필터(최초/1회유찰/2회유찰/3회이상),
  최저매각가 금액대별 필터, 물건 마커(최저매각가), 목록, 상세정보, 카카오 로드뷰,
  현황조사서·감정평가서 바로 보기, AI 권리분석
...
- **매각물건명세서 요약**: 법원이 발급하는 원본 PDF는 문서뷰어(ecfs.scourt.go.kr)가 발급하는
  서명 토큰(`encParam`)이 **그 토큰을 발급받은 courtauction.go.kr 브라우저 세션에만 유효**해서
  우리 서버가 대신 열어줄 수 없다 (Referer 문제가 아니라 세션 바인딩 문제 - 실제로 같은 브라우저의
  courtauction.go.kr 탭에서 `window.open`으로 열어도 우리 서버가 만든 토큰이면 똑같이 로그인
  화면이 뜨는 것을 확인했다). 대신 매각물건명세서를 구성하는 것과 동일한 공개 데이터
  (`selectAuctnCsSrchRslt.on`의 최선순위 설정일자·말소되지 않는 권리 등 + `selectCurstExmndc.on`의
  점유관계·임차인 현황 - AI 권리분석에 쓰는 것과 같은 데이터)를 모아 화면에 즉시 요약으로
  보여준다. 법원 발급 원본(PDF) 자체가 필요하면 요약 모달의 "법원 발급 원본 보기" 버튼이
  사건번호를 클립보드에 복사하고 공식 검색 화면을 새 창으로 열어, 그 자리에서 수동으로
  검색하도록 안내한다.

문서 조회는 매각기일 기준 1~2주 전부터만 가능하다는 법원 사이트의 제약이 그대로 적용된다
