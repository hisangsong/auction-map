...
이 문서들은 대법원 법원경매정보 사이트가 별도의 문서뷰어 시스템(ecfs.scourt.go.kr)을
통해 클릭할 때마다 임시 서명 링크로 제공합니다. 사건별로 미리 발급받아 저장해두는 방식은
링크가 곧 만료되어 정적 사이트에 적합하지 않으므로, 상세 패널의 "원본보기" 버튼은 대신
- 사건번호를 클립보드에 복사하고
- 공식 사이트의 물건상세검색 화면을 새 창으로 엽니다.

사용자가 그 화면에서 법원 선택 후 사건번호를 붙여넣어 검색하면, 원본 문서 버튼
(매각물건명세서/현황조사서/감정평가서)을 바로 이용할 수 있습니다.
